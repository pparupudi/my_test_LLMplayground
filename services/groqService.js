...
class GroqService {
   ...
   */
  async generateCompletion({ prompt, temperature = 0.7, max_tokens = 1000, top_p = 1, model = 'llama-3.1-8b-instant' }) {
    try {
      // Validate parameters
      ...
      if (error.response?.status === 401) {
        ...
        throw new AppError(`Groq API 404 error: ${error.response.data?.error?.message || 'Endpoint not found'}`, 404);
